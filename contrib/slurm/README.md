...
The intended workflow is to have a **single working directory** containing your benchmark XML, tool binaries, input files, etc. The generate step creates a `slurm/` subdirectory inside it; you transfer the whole directory to the HPC, run the jobs, and transfer the results back.

### Step 1: Generate the SLURM jobs (local)
...
This creates `./slurm/<runset>/` containing:

```
manifest.json       # All task command lines and resource limits
slurm_worker.py     # Worker script (copied in, no external dependencies)
job.sh              # sbatch script (uses relative paths only)
```

### Step 2: Transfer the working directory to HPC

```bash
rsync -a . hpc-login:~/my-benchmark/
```

This transfers everything — tools, inputs, benchmark files, and the generated `slurm/` directory.

### Step 3: Submit on HPC

```bash
ssh hpc-login
cd ~/my-benchmark/slurm/<runset>/
sbatch job.sh
```
...
```bash
rsync -a hpc-login:~/my-benchmark/slurm/ ./slurm/
```
...
```
my-benchmark/           # Your working directory
  benchmark.xml         # Benchmark definition
  tool-binary           # Tool under test
  inputs/               # Input files
  slurm/                # Generated by --slurm-mode=generate
    <runset>/
...
3. **Command paths must match on both machines** — the manifest records command lines as generated locally. Since the entire working directory is transferred, relative paths work naturally.
